                      ...
                      return (
                        <div key={j.id} onClick={() => setSelectedJob(j)}
                          onContextMenu={(e) => openMenu(e, [
                            ...
                            { label: "Copy Customer", icon: "👤", action: () => navigator.clipboard.writeText(`${j.customerName || "Walk-in"}\n${j.customerPhone || ""}`) },
                            ...(j.status === "pending" ? [{ label: "Assign Driver", icon: "🚗", action: () => {} }] : []),
                            ...(j.status !== "completed" && j.status !== "cancelled" ? [{ label: "Mark Complete", icon: "✅", action: () => {} }] : []),
                            { label: "Delete Job", icon: "🗑️", action: () => {}, variant: "danger" as const },
                          ])}
                          className={`bg-white border rounded-lg p-4 cursor-pointer card-hover ${selectedJob?.id === j.id ? "border-[#533afd] shadow-[0_4px_12px_rgba(83,58,253,0.1)]" : "border-[#e5edf5]"}`}>
